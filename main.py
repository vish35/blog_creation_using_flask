...
with open('config.json', 'r') as c:
    params: object = json.load(c)["params"]
...
def edit(sno):
    if 'user' in session and session['user'] == params['admin_user']:
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=box_title, tagline=tline, slug=slug, content=content, date=date, by=by)
                db.session.add(post)
                db.session.commit()
            else:
                post = Posts.query.filter_by(sno=sno).first()
...
def contact():
    if request.method == 'POST':
        ...
        entry = Contact(name=name, email=email, phoneno=phone, date=datetime.now(), msg=message)
